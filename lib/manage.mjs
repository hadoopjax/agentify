...
function recentIssueEvents() {
  ...
  if (fs.existsSync(eventsPath)) {
    ...
    const controlPlane = path.resolve(path.join(path.dirname(new URL(import.meta.url).pathname), "control_plane.py"));
    const result = spawnSync("python3", [controlPlane, "event-list", "--after", "0"], {
      cwd: repoDir,
      env: process.env,
      encoding: "utf8",
    });
  ...
  const lines = events.filter(ev => typeof ev.msg === "string" && identifiers.some(identifier => ev.msg.includes(identifier)));
